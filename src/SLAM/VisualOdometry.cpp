...
VisualOdometry::VisualOdometry(int argc, char **argv, NetworkManager *network, ApplicationState &app,
                               DataManager *data) {
    ...
    CLAY_LOG_INFO("Params: {} {} {} {}", data->GetLeftCamera()._fx, data->GetLeftCamera()._cx,
                  data->GetLeftCamera()._fy, data->GetLeftCamera()._cy);
...
void
VisualOdometry::ExtractKeypoints(cv::Mat img, cv::Ptr<cv::ORB> orb, std::vector<cv::KeyPoint> &points, cv::Mat &desc) {
    desc.setTo(0);
    points.clear();
...
void VisualOdometry::TrackKeypoints(cv::Mat prev, cv::Mat cur, std::vector<cv::Point2f> &prev_pts,
...
void VisualOdometry::TriangulateStereoPoints(cv::Mat &leftPoseWorld, std::vector<cv::KeyPoint> kpLeft,
                                             std::vector<cv::KeyPoint> kpRight, std::vector<cv::DMatch> stereoMatches,
...
void
VisualOdometry::TriangulateStereoNormal(std::vector<cv::KeyPoint> &pointsTrain, std::vector<cv::KeyPoint> &pointsQuery,
                                        std::vector<cv::DMatch> &matches, std::vector<PointLandmark> &points3D,
                                        float baseline, float focalLength) {
    points3D.clear();
...
void VisualOdometry::ExtractFinalSet(std::vector<cv::DMatch> leftMatches, std::vector<cv::KeyPoint> curLeftKp,
                                     std::vector<PointLandmark> &points3D) {
    int overlap = 0;
...
//            CLAY_LOG_INFO("LeftTrain:{} -> 3DTrain:{}", match.trainIdx, points3D[i]._index[0]);
                points3D[i].AddMeasurement2D(
                        Eigen::Vector2f(curLeftKp[match.queryIdx].pt.x, curLeftKp[match.queryIdx].pt.y), match.queryIdx,
                        2);
                overlap++;
            }
        }
...
const Eigen::Matrix4f &VisualOdometry::EstimateMotion(std::vector<PointLandmark> points, int cameraID) {
    ...
    for (auto landmark: points) {
        ...
        objectPoints.emplace_back(
                cv::Point3f(landmark.GetPoint3D().x(), landmark.GetPoint3D().y(), landmark.GetPoint3D().z()));
    }
...
//   CLAY_LOG_INFO("(Visual Odometry) Total Time Taken: {} ms\n", duration);
    ...
    for (int i = 0; i < prevPoints3D.size(); i++) {
        cloud->InsertVertex(prevPoints3D[i].GetPoint3D().x() * 0.01, prevPoints3D[i].GetPoint3D().y() * 0.01,
                            prevPoints3D[i].GetPoint3D().z() * 0.01);
    }
...
cv::Mat
VisualOdometry::EstimateMotion_2D2D(std::vector<cv::Point2f> &prevFeatures, std::vector<cv::Point2f> &curFeatures,
                                    cv::Mat &mask, const CameraParams &cam) {
    using namespace cv;
...
cv::Mat VisualOdometry::TriangulatePoints(std::vector<cv::Point2f> &prevPoints, std::vector<cv::Point2f> &curPoints,
                                          const CameraParams &cam, cv::Mat relativePose) {
    using namespace cv;
    float data[9] = {cam._fx, 0, cam._cx, 0, cam._fy, cam._cy, 0, 0, 1};
...
void VisualOdometry::CalculateOdometry_FAST(ApplicationState &appState, Eigen::Matrix4f &transform) {
    ...
    */
    ...
    if (appState.DATASET_ENABLED) {
        ...
        ((ImageReceiver *) this->_dataReceiver->receivers[appState.KITTI_LEFT_IMG_RECT])->getData(leftImage, appState,
                                                                                                  timestamp);
        ((ImageReceiver *) this->_dataReceiver->receivers[appState.KITTI_RIGHT_IMG_RECT])->getData(rightImage, appState,
                                                                                                   timestamp);
    }

    if (!leftImage.empty() && leftImage.rows > 0 && leftImage.cols > 0 && !rightImage.empty() && rightImage.rows > 0 &&
        rightImage.cols > 0) {
