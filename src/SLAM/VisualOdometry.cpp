...
cv::Mat VisualOdometry::EstimateMotion_2D2D(std::vector<cv::Point2f>& prevFeatures, std::vector<cv::Point2f>& curFeatures, cv::Mat& mask, const CameraParams& cam)
{
   using namespace cv;
...
cv::Mat VisualOdometry::TriangulatePoints(std::vector<cv::Point2f>& prevPoints, std::vector<cv::Point2f>& curPoints, const CameraParams& cam, cv::Mat relativePose)
{
   using namespace cv;
   float data[9] = {   cam._fx, 0, cam._cx, 0, cam._fy, cam._cy, 0, 0, 1 };
...
void VisualOdometry::Update(ApplicationState& appState, Clay::Ref<Clay::TriangleMesh> axes, Clay::Ref<Clay::PointCloud> cloud)
{
   auto start_point = std::chrono::steady_clock::now();
