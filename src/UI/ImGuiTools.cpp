...
void ImGuiTools::ScatterPlotXY(const std::vector<Eigen::Vector2f>& points, const std::string& label, bool lines, bool loop)
{
   ...
   if(lines) ImPlot::PlotLine(label.c_str(), x_data.data(), y_data.data(), points.size() + (loop ? 1 : 0));
}
