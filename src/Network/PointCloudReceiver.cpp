...
/* Reference:
 * https://answers.ros.org/question/285625/accessing-values-while-iterating-points-in-sensor_msgspointcloud2-from-sensor_msgspointcloud2constiterator/
 * */
void PointCloudReceiver::PointCloud2Callback(const sensor_msgs::PointCloud2ConstPtr& cloudMsg)
{
   ...
   int count = 0;
   for (sensor_msgs::PointCloud2ConstIterator<float> it(*_msg, "x"); it != it.end(); ++it)
   {
...
//   }
   ...
   CLAY_LOG_INFO("Clouds: {} {} {}", topicName, _clouds.size(), _cloudToRender->GetMesh()->_vertices.size());
   if (_clouds.size() > 2)
      _clouds.erase(_clouds.end());
