...
//    printf("---------------------VISIT-----------(%d,%d)\n", x,y);
    ...
    if(app.SHOW_PATCHES)circle(debug, Point((y)*app.PATCH_HEIGHT, (x)*app.PATCH_WIDTH), 2,
...
//                printf("---------------------REACHED-----------(%d,%d,%d)\n", x,y, num);
    ...
    if (count != 8) {
        ...
        if(app.SHOW_BOUNDARIES) circle(debug, Point((y)*app.PATCH_HEIGHT, (x)*app.PATCH_WIDTH), 2, Scalar(255,255,255), -1);
    }
    if(app.VISUAL_DEBUG) displayDebugger(app.VISUAL_DEBUG_DELAY);
}
...
void MapFrameProcessor::boundary_dfs(int x, int y, int component, int& num, Mat& debug, shared_ptr<RegionRing> regionRing){
    ...
    if(app.SHOW_BOUNDARIES) circle(debug,
                                      Point((y)*app.PATCH_HEIGHT, (x)*app.PATCH_WIDTH), 2,
