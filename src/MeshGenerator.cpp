...
Trade::MeshData MeshGenerator::getPlanarRegionMesh(shared_ptr<PlanarRegion> planarRegion) {
    ...
    Containers::StridedArrayView1D<Vector3> positions{vertexData,reinterpret_cast<Vector3*>(vertexData.data() + attributeOffset),
    ...
    Containers::StridedArrayView1D<Vector3> normals{vertexData,reinterpret_cast<Vector3*>(vertexData.data() + attributeOffset),
                                                    circularPoints.size() + 2, std::ptrdiff_t(stride)};
...
void MeshGenerator::getPlanarRegionBuffer(shared_ptr<PlanarRegion> planarRegion, GL::Buffer& bufferToPack) {
    ...
    positions.emplace_back(Vector3(planarRegion->getCentroid().x(), planarRegion->getCentroid().y(),
                                   planarRegion->getCentroid().z()));
