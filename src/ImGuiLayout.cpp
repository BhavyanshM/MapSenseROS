...
void ImGuiLayout::getImGui2DLayout(ApplicationState& appState, uint8_t& displayItem) {
    if(ImGui::Button("Input Color")) displayItem = 3;

    if(ImGui::Button("Input Depth")) displayItem = 1;

    if(ImGui::Button("Filtered Depth")) displayItem = 2;
    ImGui::SameLine(180);
    ImGui::Checkbox("Graph", &appState.SHOW_GRAPH);

    if(ImGui::Button("Region Components")) displayItem = 0;
    ImGui::Checkbox("Boundary", &appState.SHOW_BOUNDARIES);
    ImGui::Checkbox("Internal", &appState.SHOW_PATCHES);

    if(ImGui::Button("Hide Display")) { destroyAllWindows();destroyAllWindows();displayItem = -1; }
...
void ImGuiLayout::getImGui3DLayout(ApplicationState& appState, Color4& color) {
    if (ImGui::ColorEdit3("Color", color.data())) { GL::Renderer::setClearColor(color); }
